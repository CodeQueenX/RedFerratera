...
<div class="mt-3 p-3 border rounded">
    ...
    <p><i data-lucide="alert-circle"></i> <strong>Estado:</strong> 
        <span class="badge <?= ($ferrata['estado'] === 'Abierta') ? 'bg-success' : (($ferrata['estado'] === 'Cerrada') ? 'bg-warning' : (($ferrata['estado'] === 'No operativa') ? 'bg-danger' : (($ferrata['estado'] === 'Precaución') ? 'bg-warning' : ''))) ?>">
    ...
    ?>
        ...
        <div id="windy-widget" class="weather-widget">
            <div data-windywidget="map"
                 ...
                 data-tempunit="C"
                 data-windunit="kmh"
                 data-appid="widgets_491fa80be5"></div>
        </div>
    ...
    $videos = $videoModel->obtenerVideosPorFerrata($ferrata_id);
    if (!empty($videos)):
    ?>
    ...
    $wikilocs = $wikilocModel->obtenerWikilocPorFerrata($ferrata_id);
    if (!empty($wikilocs)):
    ?>
...
<div class="botones-acciones d-flex gap-2">
    ...
    <?php if (isset($_SESSION['usuario']) && $_SESSION['usuario']['rol'] === 'admin'): ?>
        <a href="/RedFerratera/editar-ferrata/<?= $ferrata_id; ?>" class="btn btn-warning">Editar Ferrata</a>
        <form action="/RedFerratera/index.php?accion=eliminar_ferrata&id=<?= $ferrata_id; ?>" method="POST" class="d-inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar esta ferrata?')">
    ...
    <ul class="list-group">
        <?php foreach ($ferratasCercanas as $cercana): ?>
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <span><strong><?= htmlspecialchars($cercana['nombre']); ?></strong> (<?= htmlspecialchars($cercana['distancia']) ?> km)</span>
...
$ratingData = $valoracionModel->getAverageRating($ferrata_id);
...
<div class="rating-section">
    ...
    <?php if (isset($_SESSION['usuario']) && $_SESSION['usuario']['verificado'] == 1): ?>
        <div id="starRating" data-ferrata-id="<?= $ferrata_id; ?>"></div>
        ...
        <?php foreach ($comentarios as $comentario): ?>
            <li class="list-group-item">
                ...
                <?php if (isset($_SESSION['usuario']) && $_SESSION['usuario']['id'] === $comentario['usuario_id']): ?>
                    ...
                    <a href="/RedFerratera/eliminar-comentario/<?= $comentario['id']; ?>/ferrata/<?= $ferrata_id; ?>" class="btn btn-danger btn-sm">🗑 Eliminar</a>
